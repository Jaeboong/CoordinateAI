...
async function collectMarkdownFiles(repoRoot: string): Promise<string[]> {
  ...
  for (const directory of [path.join(repoRoot, "docs", "development"), path.join(repoRoot, "docs", "runbooks")]) {
...
function resolveAbsoluteRepoPath(repoRoot: string, absoluteTarget: string): string | undefined {
  ...
  for (const marker of ["/docs/", "/src/", "/agents/", "/scripts/", "/media/", "/.github/"]) {
  ...
  for (const singleton of ["/README.md", "/AGENTS.md", "/package.json", "/tsconfig.json"]) {
