...
가장 먼저 `Providers` 탭에서 각 CLI 상태를 확인합니다.
...
설치 후에도 `Providers` 탭에서 안 보이면 VS Code를 완전히 껐다가 다시 열고, Extension Host를 다시 실행하세요. 로그인 셸의 PATH가 새로 반영되어야 할 수 있습니다.

### 2. Profile 탭

`Profile` 탭에는 모든 프로젝트에 공통으로 쓰일 자료를 넣습니다.
